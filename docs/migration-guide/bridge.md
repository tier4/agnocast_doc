...
The bridge manager node is named `agnocast_bridge_node_<ipc_ns_inode>_<pid>` by default, which is unique on every launch. When tools reference the node name in static configuration files (e.g. CIE thread configuration), set the [`AGNOCAST_BRIDGE_NODE_NAME_SUFFIX`](../api/environment-variables.md#agnocast_bridge_node_name_suffix) environment variable to name the node `agnocast_bridge_node_<suffix>` instead. The deployment configuration is responsible for choosing a value that is unique within the ROS 2 domain, such as the container name:
